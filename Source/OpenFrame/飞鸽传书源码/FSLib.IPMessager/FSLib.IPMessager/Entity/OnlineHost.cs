...
namespace FSLib.IPMessager.Entity
{
	...
	public class OnlineHost : Dictionary<string, Host>
	{
		...
		public OnlineHost(Config config/*, Network.MessageTranslator messageProxy*/)
		{
			...
			sopHostAdd = (s) =>
			{
				OnHostOnline(s as HostEventArgs);
			};
			sopHostRemove = (s) =>
			{
				OnHostOffline(s as HostEventArgs);
			};
			sopHostClear = (s) =>
			{
				OnHostCleared(s as EventArgs);
			};
		}
		...
		public new void Add(string ipaddress,Host host)
		{
			if (!this.ContainsKey(ipaddress))
			{
				...
				ctx.Post(sopHostAdd, new HostEventArgs(host));
			}
		}
		...
		public Host Delete(string ipaddress)
		{
			if (this.ContainsKey(ipaddress))
			{
				...
				ctx.Post(sopHostRemove, new HostEventArgs(h));
		...
		{
			base.Clear();
			ctx.Post(this.sopHostClear, new EventArgs());
		...
		protected virtual void OnHostOnline(HostEventArgs e)
		{
			...
			if (HostOnline != null) HostOnline(this, e);
		}
		...
		{
			if (HostOffline != null) HostOffline(this, e);
		}
		...
		/// </summary>
		/// <param name="e">参数</param>
		protected virtual void OnHostCleared(EventArgs e)
		{
			if (HostCleared != null) HostCleared(this, e);
		}
