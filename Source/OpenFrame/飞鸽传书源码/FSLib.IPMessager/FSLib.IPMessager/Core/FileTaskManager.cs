...
namespace FSLib.IPMessager.Core
{
	...
	public class FileTaskManager : IDisposable
	{
		...
		/// 请求接收文件
		/// </summary>
		public event EventHandler<FileReceiveEventArgs> FileReceiveRequired;

		/// <summary>
		/// 正在请求接收文件
		/// </summary>
		public event EventHandler<FileReceiveEventArgs> FileReceiveRequiring;

		/// <summary>
		/// 触发请求接收文件事件
		/// </summary>
		/// <param name="e">事件参数</param>
		void OnFileReceiveRequired(FileReceiveEventArgs e)
		{
		...
		/// </summary>
		/// <param name="e"></param>
		protected virtual void OnFileReceiveRequiring(FileReceiveEventArgs e)
		{
			...
			foreach (FileTaskInfo t in ReceiveTask)
			{
				for (int j = 0; j < t.TaskList.Count; j++)
				{
					if (t.TaskList[j].State == FileTaskItemState.Scheduled)
					{
						...
						FileReceiveEventArgs e = new FileReceiveEventArgs(task, item);
