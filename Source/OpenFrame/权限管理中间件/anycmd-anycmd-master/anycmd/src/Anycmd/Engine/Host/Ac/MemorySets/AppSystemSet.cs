    ...
    internal sealed class AppSystemSet : IAppSystemSet, IMemorySet
    {
        ...
        public AppSystemState SelfAppSystem
        {
            get
            {
                ...
                {
                    throw new GeneralException("尚未配置SelfAppSystemCode，在AcDomain初始化时由传入的IAppConfig对象配置。");
                }
                else if (!_dicByCode.ContainsKey(_acDomain.Config.SelfAppSystemCode))
                {
                    throw new GeneralException("意外的SelfAppSystemCode：" + _acDomain.Config.SelfAppSystemCode);
                }
                else
                {
        ...
        private void Init()
        {
            ...
            lock (Locker)
            {
                ...
                foreach (var appSystem in appSystems)
                {
                    ...
                    {
                        throw new GeneralException("意外重复的应用系统编码" + appSystem.Code);
                    }
                    if (_dicById.ContainsKey(appSystem.Id))
                    {
                        throw new GeneralException("意外重复的应用系统标识" + appSystem.Id);
                    }
            ...
            IHandler<RemoveAppSystemCommand>
        {
            ...
            private void Handle(IAcSession acSession, IAppSystemCreateIo input, bool isCommand)
            {
                ...
                var repository = acDomain.RetrieveRequiredService<IRepository<AppSystem, Guid>>();
                ...
                {
                    throw new GeneralException("标识是必须的");
                }
                AppSystem entity;
                lock (MessageLocker)
                {
                    ...
                    {
                        throw new GeneralException("给定标识的记录已经存在" + input.Id);
                    }
