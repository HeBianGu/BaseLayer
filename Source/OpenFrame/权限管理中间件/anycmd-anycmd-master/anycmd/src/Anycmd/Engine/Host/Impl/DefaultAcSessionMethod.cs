...
namespace Anycmd.Engine.Host.Impl
{
    ...
    public class DefaultAcSessionMethod : IAcSessionMethod
    {
        ...
        private RdbDescriptor GetAccountDb(IAcDomain acDomain)
        {
            ...
            {
                throw new GeneralException("意外的实体类型码Ac.Account");
            }
            RdbDescriptor db;
            if (!acDomain.RdbSet.TryDb(entityType.DatabaseId, out db))
            {
                throw new GeneralException("意外的账户数据库标识" + entityType.DatabaseId);
            }
            return db;
        }
