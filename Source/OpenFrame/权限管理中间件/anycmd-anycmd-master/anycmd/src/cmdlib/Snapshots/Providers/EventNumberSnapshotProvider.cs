...
namespace Anycmd.Snapshots.Providers
{
    ...
    /// </summary>
    public class EventNumberSnapshotProvider<TAggregateRootId> : StorageBasedSnapshotProvider<TAggregateRootId>
    {
        ...
        sealed class EventNumberSnapshotMappingKey : IEquatable<EventNumberSnapshotMappingKey>
        {
            ...
            private readonly TAggregateRootId _id;
            #endregion
            ...
            public EventNumberSnapshotMappingKey(string typeName, TAggregateRootId id)
            {
            ...
            public static bool operator ==(EventNumberSnapshotMappingKey a, EventNumberSnapshotMappingKey b)
            {
                ...
                if (((object)a == null) || ((object)b == null))
                    return false;
                return a.Id.ToString() == b.Id.ToString() && a.TypeName == b.TypeName;
            }
            public static bool operator !=(EventNumberSnapshotMappingKey a, EventNumberSnapshotMappingKey b)
            {
                return !(a == b);
            ...
            public bool Equals(EventNumberSnapshotMappingKey other)
            {
                ...
                if (other == null)
                    return false;
                return this._id.Equals(other.Id) && this._typeName == other.TypeName;
            }
            #endregion
        }
        #endregion
        ...
        private readonly Dictionary<EventNumberSnapshotMappingKey, ISnapshot<TAggregateRootId>> _snapshotMapping = new Dictionary<EventNumberSnapshotMappingKey, ISnapshot<TAggregateRootId>>();
        ...
        public override bool CanCreateOrUpdateSnapshot(ISourcedAggregateRoot<TAggregateRootId> aggregateRoot)
        {
            ...
            else
            {
                ...
                ISpecification<DomainEventDataObject<TAggregateRootId>> spec = Specification<DomainEventDataObject<TAggregateRootId>>.Eval(
                    p => p.SourceId.Equals(aggregateRootId) &&
                        p.AssemblyQualifiedSourceType == aggregateRootTypeName &&
                        p.Version <= version);
                var eventCnt = this.EventStorage.GetRecordCount<DomainEventDataObject<TAggregateRootId>>(spec);
        ...
        public override ISnapshot<TAggregateRootId> GetSnapshot(Type aggregateRootType, TAggregateRootId id)
        {
            ...
            ISpecification<SnapshotDataObject<TAggregateRootId>> spec = Specification<SnapshotDataObject<TAggregateRootId>>.Eval(
                p => p.AggregateRootType == aggregateRootTypeName && p.AggregateRootId.Equals(id));
            var dataObj = this.SnapshotStorage.SelectFirstOnly<SnapshotDataObject<TAggregateRootId>>(spec);
            if (dataObj == null)
                return null;
            var snapshot = _snapshotSerializer.ExtractSnapshot(dataObj);
        ...
        public override bool HasSnapshot(Type aggregateRootType, TAggregateRootId id)
        {
            ...
            ISpecification<SnapshotDataObject<TAggregateRootId>> spec = Specification<SnapshotDataObject<TAggregateRootId>>.Eval(
                p => p.AggregateRootType == aggregateRootTypeName && p.AggregateRootId.Equals(id));
            var snapshotRecordCnt = this.SnapshotStorage.GetRecordCount<SnapshotDataObject<TAggregateRootId>>(spec);
