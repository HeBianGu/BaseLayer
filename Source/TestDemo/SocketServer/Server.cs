...
namespace SocketServer
{
    ...
    {
        #region TCPServer服务器
        private IEngineServer server = null;
        /// <summary>
        /// 当接收到来之客户端的文本信息的时候
        /// </summary>
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                server = SocketEngineService.CreateServer(int.Parse(textBox_port.Text));
                server.AcceptString += new Action<IPEndPoint, string>(acceptString);
                server.AcceptByte += new Action<IPEndPoint, byte[]>(acceptBytes);
                server.Connect += new Action<IPEndPoint>(connect);
                server.DateSuccess += new Action<IPEndPoint>(dateSuccess);
                server.Disconnection += new Action<IPEndPoint, string>(disconnection);
                server.EngineClose += new Action(engineClose);
                server.EngineLost += new Action<string>(engineLost);
