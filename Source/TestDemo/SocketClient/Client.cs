...
using System.Net;
using HebianGu.ComLibModule.SocketHelper;
...
namespace SocketClient
{
    ...
    {
        #region TCP客户端区
        private IEngineClient TxClient = null;
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                TxClient = SocketEngineService.CreateClient(textBox2.Text, int.Parse(textBox3.Text));
                TxClient.AcceptString += new Action<IPEndPoint, string>(accptString);//当收到文本数据的时候
                TxClient.DateSuccess += new Action<IPEndPoint>(sendSuccess);//当对方已经成功收到我数据的时候
                TxClient.EngineClose += new Action(engineClose);//当客户端引擎完全关闭释放资源的时候
                TxClient.EngineLost += new Action<string>(engineLost);//当客户端非正常原因断开的时候
                TxClient.ReconnectionStart += new Action(reconnectionStart);//当自动重连开始的时候
                TxClient.StartResult += new Action<bool, string>(startResult);//当登录完成的时候
        ...
        {
            udptx = SocketEngineService.CreateUdp();
            udptx.Port = 13091;//如果要持续的进行监听,请在这里设置好非0的监听端口，一次性的话可以不用设置
            udptx.AcceptString += new Action<IPEndPoint, string>(UdpAcceptString);
            udptx.DateSuccess += new Action<IPEndPoint>(UdpDateSuccess);
            udptx.EngineClose += new Action(engineClose);
            udptx.EngineLost += new Action<string>(engineLost);
            udptx.StartEngine();
