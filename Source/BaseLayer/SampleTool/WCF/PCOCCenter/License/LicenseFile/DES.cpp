...
CString CDES::EncryString(CString strOriginal, CString strKey)
{
	InitializeKey(strKey.GetBuffer(strKey.GetLength()),0);

	// 加密字符串
	EncryptAnyLength(strOriginal.GetBuffer(strOriginal.GetLength()),sizeof(char)*strOriginal.GetLength(),0);
	return GetCiphertextHexStr();
}

CString CDES::DecryString(CString strCiphertext, CString strKey)
{
	InitializeKey(strKey.GetBuffer(strKey.GetLength()),0);

	int nBuffLen = strCiphertext.GetLength()/2+1;
	char * buffBytes = new char[nBuffLen];
	memset(buffBytes, 0, sizeof(char)*nBuffLen);
	// 转换十六进制字符串为char*密文
	HexStrToBytes(strCiphertext, buffBytes, nBuffLen);

	// 解密字符串
	DecryptAnyLength(buffBytes, nBuffLen, 0);
	delete[] buffBytes;
...
char CDES::ConvertHexChar(char ch)
{
	if((ch>='0')&&(ch<='9'))   
		return   ch-0x30;   
	else   if((ch>='A')&&(ch<='F'))   
...
/**
     ...
     * @return byte[]
*/
bool CDES::HexStrToBytes(CString strHexSrc, char *bytes, UINT lenbytes) { 
...
/**
     ...
     * @return String
*/
CString CDES::BytesToHexStr(char *bytes, int lenbytes)
{
...
CString CDES::GetCiphertextHexStr()
{
	return BytesToHexStr(szFCiphertextAnyLength, lenCiphertext);
}
