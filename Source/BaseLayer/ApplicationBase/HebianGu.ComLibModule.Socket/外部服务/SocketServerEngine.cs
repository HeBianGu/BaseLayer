 ...
 * ========================================================================
 *    
 * 作者：[李海军]   时间：2017/1/7 17:08:06
 * 文件名：SocketServerEngine
 * 说明：
 * 
 * 
 * 修改者：           时间：               
...
namespace HebianGu.ComLibModule.SocketHelper
{
    ...
    /// <summary> 面向服务器的主线程类 </summary>
    public class SocketServerEngine : TcpEngineBase, IEngineServer
    {
        #region - 基本属性区块 -
        //  所有客户端
        private List<TcpState> state = null;
        ...
        //  允许最多客户端数
        private int _clientMax = 20;

        /// <summary> 当有客户连接成功的时候,触发此事件 </summary>
        public event Action<IPEndPoint> Connect;

        /// <summary> 当有客户突然断开的时候,触发此事件,文本参数是代表断开的原因 </summary>
        public event Action<IPEndPoint, string> Disconnection;
        ...
        List<IPEndPoint> IEngineServer.ClientAll
        {
            get
            {
        ...
        internal SocketServerEngine(int port)
        {
            Port = port;
        ...
        /// <summary> 启动服务器,如果没出现异常,说明启动成功 </summary>
        override public void StartEngine()
        {
            if (EngineStart) return;
        ...
        /// <summary> 当连接一个客户端之后的回调函数 </summary>
        ...
        private void loginInitialization(object stateOne1)
        {
            ...
            while (true)//如果2秒中之内客户端没有根据登录信息回复，将自动关闭这个客户端
            {
                ...
                if (stateOne == null || stateOne.ConnectOk == true) break;
        ...
        /// <summary> 关闭服务器,释放所有资源 </summary>
        override public void CloseEngine()
        {
            try
            {
                if (HeartThread != null) HeartThread.Abort();
        ...
        {
            if (stateOne == null || ClientNumber == 0) return;
        ...
        public int SendFile(IPEndPoint ipEndPoint, string fileName)
        {
            ...
            {
                throw new Exception("此IPEndPoint不存在");
            }
            try
            {
        ...
        /// <summary> 把ip地址转化成TcpState </summary>
