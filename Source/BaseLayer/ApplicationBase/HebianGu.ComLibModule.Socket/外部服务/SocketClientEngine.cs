 ...
 * ========================================================================
 *    
 * 作者：[李海军]   时间：2017/1/7 17:06:00
 * 文件名：SocketClientEngine
 * 说明：
 * 
 * 
 * 修改者：           时间：               
...
{
    /// <summary> 面向客户端的主线程类 </summary>
    public class SocketClientEngine : TcpEngineBase, IEngineClient
    {
        #region - 基本属性区块 -
        ...
        /// <summary> 自动重连开始的时候,触发此事件 </summary>
        public event Action ReconnectionStart;

        /// <summary> 客户端登录成功或失败都会触发此事件,登录失败的话会有失败的原因 </summary>
        public event Action<bool, string> StartResult;

        /// <summary> 当连接断开时是否重连,默认重连 </summary>
        ...
        /// <summary> 登录超时时间设置，默认10秒 </summary>
        public int OutTime
        {
            get { return _outTime; }
            set { _outTime = value; }
        ...
        internal SocketClientEngine(string ip, int port)
        {
            Ip = ip;
            Port = port;
        }
        ...
        #region - 启动客户端区块 -
        /// <summary>启动客户端,设置超时间在OutTime里设置,无论失败或成功都会触发StartResult事件 </summary>
        ...
        /// <summary>客户端向服务器发送图片数据</summary>
        public void sendMessage(byte[] data)
        {
            sendMessage(stateOne, data);
        }

        /// <summary> 客户端向服务器发送文本数据</summary>
        ...
        /// <summary> 关闭相连的scoket以及关联的StateObject,释放所有的资源 </summary>
        ...
        /// <summary>当客户端突然与服务器断开的时候</summary>
        private void lostClient(string str)
        {
            if (stateOne.ConnectOk == true) return;//说明这个引擎已经触发关闭了;下面的就不用执行了
