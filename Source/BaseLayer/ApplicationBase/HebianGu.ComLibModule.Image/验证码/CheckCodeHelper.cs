...
namespace HebianGu.ComLibModule.ImageHelper
{
    ...
    class CheckCodeHelper
    {

        /// <summary> 随机创建验证图片 </summary>
        public Bitmap CreateImage(string txt)
        {
            // 下载于www51aspx.com
        ...
        /// <summary> 随机创建验证图片 </summary>
        public Tuple<Bitmap, string> CreateImage()
        {
            string str = this.CreateString();

            Bitmap image = this.CreateImage(str);

            return new Tuple<Bitmap, string>(image, str);
        }

        /// <summary> 调用函数产生4个随机中文汉字编码  </summary>
        object[] CreateCode(int strlength)
        {
            /* 
